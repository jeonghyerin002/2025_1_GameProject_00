...
public class Monster : MonoBehaviour
{
    ...
    void Start()
    {
        Health += 100;                        // 첫 프레임 이전에 실행 될 때 100체력을 추가 시켜준다.
    }
    ...
    void Update()
    {
        ...
        {
            Health -= AttackPoint;                  // 체크 포인트를 공격 포인트 만큼 감소 시켜준다. (Health = Health - AttackPoint)
        }
    ...
    public void CharacterHit(int Demage)                 // 커스텀 데미지를 받는 함수를 사용한다.
    {
        Health -= Demage;                         // 받은 공격력에 대한 체력을 감소시킨다.
    }
    void CheckDeath()                            // 함수 선언
    {
        if (Health <= 0)                           // 체력이 0이하로 내려가면 차괴 시킨다.
